...
function redimensionarESalvarImagem($arquivo, $largura = 800, $altura = 800) {
    ...
    imagecopyresampled($imagem, $imagem_original, 0, 0, 0, 0, $nova_largura, $nova_altura, $largura_original, $altura_original);

    // Salva a nova imagem
    if ($tipo_arquivo == "jpg" || $tipo_arquivo == "jpeg") {
        imagejpeg($imagem, $caminho_completo, 90);
    } elseif ($tipo_arquivo == "png") {
        imagepng($imagem, $caminho_completo);
    } elseif ($tipo_arquivo == "gif") {
        imagegif($imagem, $caminho_completo);
    }
...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if (isset($_FILES['imagem']) && $_FILES['imagem']['error'] == 0) {
        ...
        if (strpos($resultado_upload, "Erro") !== false) {
